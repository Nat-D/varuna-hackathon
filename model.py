...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, 3, 1, 1, bias=False), # nn.Conv2d(in_channels, out_channels, 3, 1, 1, bias=False),
                #nn.BatchNorm2d(out_channels), 
                nn.ReLU(inplace=True),
                nn.Conv2d(out_channels, out_channels, 3, 1, 1, bias=False), # nn.Conv2d(out_channels, out_channels, 3, 1, 1, bias=False),
                #nn.BatchNorm2d(out_channels), 
                nn.ReLU(inplace=True),
            )
