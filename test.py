...
def prediction(model, selected_days, test_data_dir, test_mask_dir, output_dir, device):
    ...
    for day in days:
        ...
        df = pd.DataFrame(data = sorted_mask_pred_np, columns = column_values)
        df.to_csv(output_dir +'output.csv', index = False)  


if __name__ == "__main__":

    PATH = 'runs/PS/my_checkpoint.pth.tar'
    DEVICE = 'cpu'
    PREPROCESS = Standardize('PS', device= DEVICE)
    MODEL = NoNameUNET(in_channels=12, out_channels=12, preprocess=PREPROCESS).to(DEVICE)
    ...
    prediction(model, SELECTED_DAYS, TEST_DATA_DIR, TEST_MASK_DIR, OUTPUT_DIR, DEVICE)
