...
def generate_val_mask(shape_path, src, test=False):
    raster_img = src.read()
        ...
        for crop_type in ['1','2','3','4']:
            if row['crop_type'] == crop_type:
                ...
                if rand_train:
                    ...
                    train_mask = rasterize(shapes=temp, out_shape=im_size)
                    cv2.imwrite(f'./train_output/img/{crop_type}/{num}.png', train_mask*255)
                    np.save(f'./train_output/mask/{crop_type}/{num}.npy', train_mask)
                    count += 1
                else:
                    poly_shp_val[crop_type].append(poly)
                    val_mask = rasterize(shapes=poly_shp_val[crop_type], out_shape=im_size)
                    cv2.imwrite(f'./val_output/img/{crop_type}/{num}.png', val_mask*255)
                    np.save(f'./val_output/mask/{crop_type}/{num}.npy', val_mask)
                    count_val += 1
...
if __name__ == "__main__":
    ...
    training_shape_path = "C:/Users/NataphopT/Desktop/ARVHackathon/VarunaHackathon/raw-data/training_area/"

    # testing_shape_path = "raw_data/testing_area/"

    with rasterio.open(raster_path, "r") as src:
        poly_shp_train, poly_shp_val = generate_val_mask(training_shape_path, src)
